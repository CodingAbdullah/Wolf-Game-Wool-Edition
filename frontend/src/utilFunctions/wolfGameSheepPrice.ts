...
export const wolfGameSheepFloorPrice = async () => {
    const response = await axios.get('https://localhost:5000/wolf-game-sheep-floor-price');
...
export const wolfGameSheepGen2FloorPrice = async () => {
    const response = await axios.get('https://localhost:5000/wolf-game-sheep-floor-gen-two-price');
